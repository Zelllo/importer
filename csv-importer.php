...
function wps_theme_func(){
	...
	if(file_get_contents($path['input'])){
		...
		echo'<span class="yellow-message">We found your file!<br>Please, enter "IMPORT" to start airports import.</span><a href="' . get_home_url() . '/wp-content/plugins/csv-importer/includes/import.php" class="update-button" >Import</a>';
	}else{
...
function airport_locate_func(){
	...
	$ip = "http://api.ipapi.com/" .$_SERVER['REMOTE_ADDR']. "?access_key=3947612504278229bb7d9e665ca2ce31";

	$details = wp_remote_get($ip);
	$details = json_decode($details['body'], true); // -> "US"
	$city = $details['city'];

	$airoport = $wpdb->get_var( "SELECT name FROM wp_airports WHERE municipality='Kyiv'" );
	$airoport_type = $wpdb->get_var( "SELECT type FROM wp_airports WHERE municipality='$city'" );
